...
static int if_addr_parse(GQueue *q, char *s, struct ifaddrs *ifas, int port_min, int port_max) {
	str name;
	char *c;
	sockaddr_t *addr, adv;
...
static void options(int *argc, char ***argv) {
	...
	int stepn = (rtpe_config.port_max - rtpe_config.port_min)/interfaces_n;

	int i = 0;
	for (iter = if_a; *iter; iter++) {
		int ret = if_addr_parse(&rtpe_config.interfaces, *iter, ifas, 
			rtpe_config.port_min+i*stepn, rtpe_config.port_min+(i+1)*stepn);
		i++;
		if (ret)
